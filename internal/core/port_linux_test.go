...
func TestPortListedIn(t *testing.T) {
	...
	if inUse, err := portListedIn(free, 67); err != nil || inUse {
		t.Errorf("portListedIn(free, 67) = %v, %v; want false, nil", inUse, err)
	}
	if inUse, err := portListedIn(busy, 67); err != nil || !inUse {
		t.Errorf("portListedIn(busy, 67) = %v, %v; want true, nil", inUse, err)
	}
...
func TestPortListedInHandlesMissingFile(t *testing.T) {
	inUse, err := portListedIn(filepath.Join(t.TempDir(), "absent"), 67)
	if err != nil || inUse {
...
func TestPortListedInIgnoresGarbage(t *testing.T) {
	...
	if inUse, err := portListedIn(path, 67); err != nil || inUse {
		t.Errorf("got %v, %v; want false, nil", inUse, err)
	}
}

// The real procfs must at least parse, for both protocols. Whether anything
// holds a given port here is not asserted — that is the host's business, not
// the test's.
func TestPortInUseReadsRealProcfs(t *testing.T) {
	if _, err := os.Stat("/proc/net/udp"); os.IsNotExist(err) {
		t.Skip("no /proc/net/udp")
	}
	if _, err := UDPPortInUse(67); err != nil {
		t.Errorf("UDPPortInUse on the live system: %v", err)
	}
	if _, err := TCPPortInUse(53); err != nil {
		t.Errorf("TCPPortInUse on the live system: %v", err)
	}
}
