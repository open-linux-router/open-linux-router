...
//
// docs/port-forwarding.md §5.2: in nftables a drop is final, so an accept in our table
...
// a warning (docs/port-forwarding.md §5.2), and a box whose procfs we cannot read is
...
func applyNFT(d Desired) error {
	...
	if len(d.Table.Masq) > 0 || d.Table.Egress != nil {
		...
		// Last in the chain, and deliberately so: a hairpinned connection must
		// be rewritten to this router's inside address, not to the uplink's.
		// NFTable.Egress has the argument.
		if r := d.Table.Egress; r != nil {
			set, elements := egressSet(table, r.Sources)
			if err := conn.AddSet(set, elements); err != nil {
				return fmt.Errorf("building the egress source set: %w", err)
			}
			conn.AddRule(&nftables.Rule{
				Table: table, Chain: post,
				Exprs:    egressExprs(*r, set),
				UserData: comment(r.Line()),
			})
		}
	}
...
//     (docs/port-forwarding.md §3.2). It also keeps the rule from capturing transit
...
// to this router's, so the device replies through us (docs/port-forwarding.md §4).
//
...
// egressSet is the source set the egress rule matches on.
//
// Anonymous (`Anonymous: true`), unlike the named counters: a set has no
// state worth preserving across a table replace, so there is nothing for an
// anonymous one to lose — which is the whole of docs/port-forwarding.md §3.4's
// argument about counters, applied in the other direction.
//
// `Interval: true` because the elements are prefixes rather than addresses; an
// nftables interval set stores each as a [start, end) pair, which is what
// AddSet expects the elements to already be.
func egressSet(table *nftables.Table, sources []netip.Prefix) (*nftables.Set, []nftables.SetElement) {
	set := &nftables.Set{
		Table:     table,
		Anonymous: true,
		Constant:  true,
	...
	elements := make([]nftables.SetElement, 0, len(sources)*2)
	for _, p := range sources {
		p = p.Masked()
		start := p.Addr().AsSlice()
		elements = append(elements,
			nftables.SetElement{Key: start},
			nftables.SetElement{Key: nextPrefix(p), IntervalEnd: true},
		)
	}
	return set, elements
}
...
// egressExprs builds the outbound masquerade: traffic from one of our networks,
// leaving by the uplink, gets the uplink's own source address.
//
// Matched on the *output* interface rather than on the input one. By
// postrouting the routing decision has been made, so `oifname` is the honest
// question — "is this packet actually leaving by the uplink" — and a packet
// that an exit sent somewhere else never matches, which is what keeps this rule
// and internal/gateway's per-exit SNAT from arguing.
func egressExprs(r EgressRule, set *nftables.Set) []expr.Any {
	return []expr.Any{
		&expr.Meta{Key: expr.MetaKeyNFPROTO, Register: 1},
		&expr.Cmp{Op: expr.CmpOpEq, Register: 1, Data: []byte{unix.NFPROTO_IPV4}},
		...
		&expr.Cmp{Op: expr.CmpOpEq, Register: 1, Data: nulTerminated(r.Out)},
