...
// docs/port-forwarding.md §5.2: this is not something we can fix, so it is reported and
...
// docs/port-forwarding.md §5.2's second row, and the one that costs the operator their
// session.
...
func TestImpactRoundTripsAsText(t *testing.T) {
	...
	// sent and never read — and `olr gateway` is a client of its own API.
