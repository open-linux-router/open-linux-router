...
// The strictness that used to be tested here moved with the document: the
// forwards are a field of gateway's config now, and internal/gateway's
// UnmarshalConfig is what rejects a mistyped key in them. Asserting it from
// this package would be asserting somebody else's rule against a struct this
// one no longer parses.
...
// A fresh install has no forwards, and that is an empty state rather than a
// failure — which is what the surfaces render as "nothing is forwarded in"
// instead of as an error.
func TestTheZeroConfigIsEmpty(t *testing.T) {
	var c Config
	if !c.Empty() || c.Enabled || !c.Egress.Empty() {
		t.Errorf("the zero config is not empty: %+v", c)
	}
}
