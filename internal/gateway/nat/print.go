...
func writeForwardsText(w io.Writer, c Config) error {
	if len(c.Forwards) == 0 {
		return cli.NoObjects(w, "port forwards", "Add one with `olr gateway add forward`.")
	}
	t := table(w)
...
func writeForwardText(w io.Writer, f Forward) error {
	...
	fmt.Fprintln(w, "\nRun `olr gateway status forwards` to see whether anything has arrived through it.")
	return nil
}
...
func writeStatusText(w io.Writer, s statusView) error {
	...
	if !s.Known {
		...
		fmt.Fprintln(w,
			"\nThe kernel does not match this configuration. Run `olr gateway show forwards --dry-run`\n"+
