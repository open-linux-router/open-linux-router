...
type Applier struct {
	...
	// LoadConfig and SaveConfig are the seam to the stored document, which
	// `gateway` owns.
	//
	// Two function fields rather than a *core.Store, because the forwards are
	// not a section of their own any more — they live inside `gateway`'s, and
	// the projection from that document into this package's Config is
	// `gateway`'s to make (gateway.Config.NAT). Holding a Store here would mean
	// two packages writing one section and each having an opinion about the
	// other's fields.
	//
	// SaveConfig receives a Config this package has already validated. It is
	// expected to fold the forwards back into the document and write it under
	// the global apply lock, exactly as it would for any other field of its own
	// config.
...
func (a Applier) Load() (Config, error) { return a.LoadConfig() }
...
type ForwardStatus struct {
	...
	// (docs/port-forwarding.md §3.6). That is worth knowing before reading a small
