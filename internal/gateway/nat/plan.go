...
// without this a plan can be sent but never read, and `olr gateway` is a client
...
// rather than touched (docs/port-forwarding.md §5.2, design.md §3.4's
...
type Plan struct {
	...
	//
	// Unlike internal/gateway's ForeignRule this never blocks. docs/port-forwarding.md
...
func classify(c Config, plan Plan, obs Observed, desired Desired) (Impact, []string) {
	...
	// docs/port-forwarding.md §5.2's second row. Reported here rather than in Validate
...
// describeConflicts is docs/port-forwarding.md §5.2's local-port warning.
//
...
// describeForeign is docs/port-forwarding.md §5.2's first row, in the doc's own words.
//
