...
// itself.
//
// The rules here are docs/port-forwarding.md §5.1's refusals, which are all the same
...
func (r Result) Err() error {
	...
	return fmt.Errorf("invalid port forwarding configuration:\n  %w", errors.Join(msgs...))
}
...
//
// The counter (docs/port-forwarding.md §3.4) is what settles it afterwards. This is
...
func validatePorts(r *Result, path string, f Forward) {
	...
	// docs/port-forwarding.md §1.3, and the most important check in this file because
...
// validateHairpin is docs/port-forwarding.md §4.1, said every time rather than once in
// a document.
//
...
// validateOverlaps is docs/port-forwarding.md §5.1's first row.
//
