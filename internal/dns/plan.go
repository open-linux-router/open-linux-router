...
func BuildPlan(b Backend, desired Config, links LinkView, reservations ReservationView, published PublishedView, obs Observed, now time.Time) (Plan, error) {
	result := Validate(desired, links, reservations)
	if err := result.Err(); err != nil {
		return Plan{Validation: result}, err
	}

	rendered, err := b.Render(desired, links, published)
	if err != nil {
		return Plan{Validation: result}, err
	}
