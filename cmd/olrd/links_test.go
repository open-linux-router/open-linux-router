...
func TestEveryConsumerSeesTheSameAdoption(t *testing.T) {
	...
	routingInfo, err := routingLinkView{facts: facts}.Interface("lan0")
	if err != nil {
		t.Fatal(err)
	}

	if !dhcpInfo.Adopted || !dnsInfo.Adopted || !routingInfo.Adopted {
		t.Errorf("adoption reached dhcp=%v dns=%v routing=%v; want all three",
			dhcpInfo.Adopted, dnsInfo.Adopted, routingInfo.Adopted)
	}
}
