...
import { useApplyForwardsChange, type ForwardsChange } from '@/features/nat/queries'
import { ApiError } from '@/lib/api'
import type { ForwardsApplyResult, ForwardsPlan } from '@/lib/api-types'
 ...
 * never refuses the change (docs/port-forwarding.md §5.2), because it may well be
 * accepting exactly this traffic in a rule olr cannot evaluate.
 */
export function useForwardsApply() {
  const apply = useApplyForwardsChange()

  /** A change the daemon held back because it would be disruptive. */
  const [confirming, setConfirming] = useState<{
    change: ForwardsChange
    plan: ForwardsPlan
  } | null>(null)

  /** The steps of the last failed apply, which the page keeps on screen. */
  const [failure, setFailure] = useState<ForwardsApplyResult | null>(null)

  async function send(change: ForwardsChange, confirm: boolean) {
    setFailure(null)
    try {
      ...
      const body = error.body as ForwardsApplyResult | undefined
  ...
  return {
    submit: (change: ForwardsChange) => send(change, false),
    confirming,
    confirm: async () => {
...
function describe(plan: ForwardsPlan): string {
